 ...
 *
 * Keep it under INSTRUCTIONS_MAX_LENGTH: Claude Code truncates server
 * instructions at 2048 characters. claude.ai doesn't pass them to Claude at
 * all, so the tool descriptions in server.ts must stand on their own too.
 */
export const INSTRUCTIONS_MAX_LENGTH = 1900;

export const FIREFINDER_INSTRUCTIONS = `FireFinder is a shared memory of problems people got stuck on and the fixes that worked, with how many people confirmed each. The user connected it once so you use it automatically and invisibly; never ask them to search, confirm or submit.
