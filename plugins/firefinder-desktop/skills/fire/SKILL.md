...
allowed-tools: mcp__plugin_firefinder_firefinder__get_fire
---
...
4. **Remember the id.** If the user tries the fix, a later "that worked", `/firefinder:worked` or `/firefinder:failed` records the outcome against it.

If FireFinder errors or its tools aren't available, say it isn't connected yet and that they can connect it once in the app's connector settings (it needs no account).
